...
void main() {
  describe('introspection', () {
    ...
    ddescribe('ngProbe', () {
      it('should retrieve ElementProbe from a node', (TestBed _) {
      ...
      it('elements should list binding expressions from child nodes', (TestBed _) {
        _.compile('<div>{{a}}<span>{{b}}</span></div>');
        _.rootScope.apply(() {
          _.rootScope.context['a'] = 'A';
          _.rootScope.context['b'] = 'B';
        });

        print(_.rootElement.outerHtml); // TODO - remove
        ElementProbe divProbe = ngProbe(_.rootElement);

        expect(divProbe.bindingExpressions).toEqual(['a']);

        // The bindingExpressions should still be listed on the element probe for the text node.
        ElementProbe textProbe = ngProbe(_.rootElement.firstChild);

        expect(textProbe.bindingExpressions).toEqual(['a']);

        ElementProbe spanProbe = ngProbe(_.rootElement.querySelector('span'));

        expect(spanProbe.bindingExpressions).toEqual(['b']);
      });

      it('should not duplicate listed bindings', (TestBed _) {
        _.compile('<div>hi {{a}}, you are {{a}}</div>');
        _.rootScope.apply(() {
          _.rootScope.context['a'] = 'A';
        });
        ...
        expect(probe.bindingExpressions.length).toBe(1);
        expect(probe.bindingExpressions).toEqual(['a']);
      });
    });
